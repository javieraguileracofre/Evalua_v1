...
def generar_cotizacion_pdf_bytes(
    *,
    ...
    kpi = [
        ...
        ["Renta neta (CLP)", _fmt_num(result.get("renta_mensual_neta") or result.get("renta_sugerida"))],
        ["Renta bruta (CLP)", _fmt_num(result.get("renta_mensual_bruta"))],
        ["Moneda cotización", str(result.get("moneda_cotizacion") or "CLP")],
        ["Renta neta (moneda)", str(result.get("renta_moneda_cotizacion") or 0)],
